...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    // 1. Save to Supabase (always succeeds)
    ...
    // 2. Send email using Gmail (Nodemailer) - with graceful failure handling
    try {
      ...
      if (!gmailPass) {
        console.warn('Email service not configured - missing GMAIL_PASS');
        return res.status(200).json({
          message: 'Contact information saved successfully (email notification disabled)',
          data,
      ...
      // Return success for Supabase storage even if email fails
      return res.status(200).json({
        message: 'Contact information saved successfully (email notification failed)',
        data,
